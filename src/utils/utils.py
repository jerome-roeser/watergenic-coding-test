...
def validate_dataframe_format(df: pd.DataFrame) -> bool:
    ...
    if not all(col in df.columns for col in required_columns):
        raise ValueError(
            "❗ DataFrame must contain the columns: time, input_variable1, input_variable2, target_variable."
        )
    ...
    if not all(col in numeric_columns for col in required_columns[1:]):
        raise ValueError(
            "❗ input_variable1, input_variable2 and target_variable must be numeric."
        )
